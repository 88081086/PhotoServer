		...
		if (files != null) {
			...
			for (i=0; i<files.length; i++) {
				if (files[i].getName().equals(photo)) {
					...
					if (photo.substring(photo.lastIndexOf('.')+1).toLowerCase().matches("jpeg|jpg")) {
						out.println(String.format("<tr><td colspan='2'><img src='%s/%s/%s' style='max-width: 100%%'/></td></tr>", Util.ROOT_PATH, album, photo));
					} else {
			...
			if (i < files.length) {
				try {
					...
					if (exif != null) {
						out.println(String.format("<tr><td colspan='2'>%s: %s(%s)</td></tr>", "设备", exif.getString(ExifIFD0Directory.TAG_MODEL), exif.getString(ExifIFD0Directory.TAG_MAKE)));
						out.println(String.format("<tr><td colspan='2'>%s: %s</td></tr>", "时间", exif.getString(ExifIFD0Directory.TAG_DATETIME)));
					}
					GpsDirectory gps = metadata.getFirstDirectoryOfType(GpsDirectory.class);
					if (gps != null) {
						GeoLocation loc = gps.getGeoLocation();
						out.println(String.format("<tr><td colspan='2'>%s: 经度%s, 纬度%s</td></tr>", "位置", GeoLocation.decimalToDegreesMinutesSecondsString(loc.getLongitude()), GeoLocation.decimalToDegreesMinutesSecondsString(loc.getLatitude())));
					}
				} catch (Exception e) {
					Log.d(""+e);
				}
